...
function Panel({ tone, label, items }: { tone: 'pos' | 'neg'; label: string; items: string[] }) {
  const pos = tone === 'pos';
  const color = pos ? '#1F6F3D' : '#9C2E2E';
  const single = items.length <= 1;
  return (
    <div
      className="rounded-md border bg-paper/60 px-4 py-3.5"
  ...
  return (
    <div className="border border-gold/30 bg-cream/30 p-6 sm:p-8 mb-12">
      ...
      <div className="grid md:grid-cols-2 gap-3">
        <Panel tone="pos" label="Strengths" items={strengthItems} />
        <Panel tone="neg" label="Weaknesses" items={weaknessItems} />
      </div>
