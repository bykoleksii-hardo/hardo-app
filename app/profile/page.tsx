              ...
              {recent.map((r) => (
                <li key={r.id} className="flex items-center justify-between gap-4 border border-[#11161E]/10 px-4 py-3 rounded-sm">
                  ...
                  <div className="text-right shrink-0">
                    {r.status === 'completed' ? (
                      <>
                        <div className="font-serif text-2xl text-[#B88736]">{r.letter_grade ?? '\u2014'}</div>
                        <div className="text-[11px] tracking-[0.18em] text-[#11161E]/55">{r.hire_recommendation ? (HIRE_LABEL[r.hire_recommendation] ?? r.hire_recommendation) : ''}</div>
                      </>
                    ) : (
                      <div className="text-[11px] tracking-[0.18em] text-[#11161E]/55">{r.status.toUpperCase()}</div>
                    )}
                  </div>
                </li>
              ))}
...
function EmptyBlock({ title, body, cta }: { title: string; body: string; cta?: { href: string; label: string } }) {
  return (
    <div className="text-center py-10">
      ...
      <p className="text-sm text-[#11161E]/60 max-w-md mx-auto mb-4">{body}</p>
      {cta && (
        <Link href={cta.href} className="inline-block bg-[#B88736] text-[#FBF7EE] font-medium tracking-[0.05em] px-6 py-2.5 rounded-sm hover:bg-[#9C6F1E] transition-colors">{cta.label}</Link>
      )}
    </div>
  );
}
