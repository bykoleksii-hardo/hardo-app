...
const steps = [
  ...
  },
  {
    ...
    body: 'Letter grade per question, six-axis skill radar, follow-up depth, and one of four hire calls.',
  ...
  if (kind === 'wave') {
    ...
    return (
      <div className="step-visual">
        <div className="wave">
          {heights.map((h, i) => (
            <i key={i} style={{ height: `${h}px`, animationDelay: `${(i % 6) * 0.12}s` }} />
          ))}
        </div>
    ...
    <section id="how" className="border-t border-line bg-cream/40">
      <div className="max-w-page mx-auto px-6 py-20">
        ...
        <div className="mt-14 grid grid-cols-1 md:grid-cols-3 border-t border-line">
          {steps.map((s, i) => (
            <Reveal key={s.n} delay={i * 130} className="step-cell">
