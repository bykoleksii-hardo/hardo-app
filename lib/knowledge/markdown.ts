...
export function renderMarkdown(md: string): string {
  ...
  // Normalize body heading depth so the outline starts at <h2> (the article
  // title is the page's only <h1>). Offset by the shallowest level the author
  // used, so articles written entirely with "###" still render proper <h2>
  // sections instead of skipping a level (h1 -> h3).
  let minHeadingLevel = 3;
  for (const l of lines) {
    const hm = /^(#{1,3})\s+\S/.exec(l);
    if (hm) minHeadingLevel = Math.min(minHeadingLevel, hm[1].length);
  }
  const headingOffset = minHeadingLevel - 2;
  ...
  while (i < lines.length) {
    ...
    if (h) {
      ...
      // Normalize so the shallowest authored heading renders as <h2> and deeper
      // levels nest beneath it; never emit a second <h1>. Visual size still keys
      // off the authored level (below), so this is a semantic-only change.
      const tagLevel = Math.min(6, Math.max(2, level - headingOffset));
